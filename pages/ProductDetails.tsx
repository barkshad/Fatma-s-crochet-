...
import { ArrowLeft, MessageCircle, RefreshCw, ShieldCheck, Loader2, Star, Share2 } from 'lucide-react';
        ...
        <div className="glass-heavy rounded-[2.5rem] overflow-hidden shadow-2xl border-0">
          <div className="grid grid-cols-1 lg:grid-cols-2">
            ...
            <motion.div 
              ...
              className="relative min-h-[500px] lg:h-auto bg-brand-brown/5"
            >
               {isVideo ? (
                 <video 
                   src={product.image} 
            ...
            <div className="p-8 md:p-12 lg:p-16 flex flex-col justify-center relative">
              <StaggerContainer delay={0.3}>
                <StaggerItem className="flex justify-between items-start mb-4">
                ...
                <StaggerItem className="prose prose-stone text-brand-text/80 mb-10 text-lg leading-relaxed">
                  <p>{product.description}</p>
                </StaggerItem>

                {/* Customization */}
                <StaggerItem className="glass p-6 rounded-2xl mb-8 border border-white/60">
                    ...
                    {['Default', 'Custom Color', 'Custom Size'].map(opt => (
                      <motion.button
                        ...
                        className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all border-2 ${
                          selectedColor === opt 
                            ? 'border-brand-sageDark bg-brand-sageDark text-white shadow-lg' 
                ...
                <StaggerItem>
                  <motion.a 
                    ...
                    whileTap={{ scale: 0.98 }}
                    className="w-full bg-[#25D366] hover:bg-[#20bd5a] text-white font-bold py-5 rounded-2xl shadow-lg hover:shadow-green-500/30 transition-colors flex items-center justify-center gap-3 mb-8 text-lg"
                  >
                ...
                {/* Details List */}
                <StaggerItem className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-brand-brown/10">
